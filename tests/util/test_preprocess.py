...
from src.util.preprocess import preprocess, train_test_split_by_step, get_column_indices, print_description

TEST_DATA = {"a": ["'col1'", "'col2'", "'fraud'"], "col2": [1, 2, 3], "fraud": [4, 5, 6]}
...
def test_train_test_split_by_step():
    ...
    # test = {"a": ["'col1'", "'col2'", "'fraud'"], "col2": [1, 2, 3], "fraud": [4, 5, 6]}
    X = pd.DataFrame.from_dict(TEST_DATA)


    result = train_test_split_by_step(X, "col2", "fraud", 0.8)
    X_train, X_test, y_train, y_test = result
    assert X_train.to_dict() == {'a': {0: "'col1'", 1: "'col2'"}, 'col2': {0: 1, 1: 2}}
...
def test_train_test_split_by_step_invalid_train_size():
    ...
    with pytest.raises(ValueError, match="train_size argument must be between 0 and 1. train_size: 1.1"):
